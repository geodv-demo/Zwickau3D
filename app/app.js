    ...
    Cesium.Ion.defaultAccessToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiI0NGZlNDBjZC0yMTdjLTQ1NjEtOWRmNS1lZGNjMmY3ODZmNjAiLCJpZCI6MTAwNzY3LCJpYXQiOjE2NzcwNjI1Nzl9.DHxqkcoLCc6yFIJjIH7YNWu9CiCoQCpIiM_wgmOV2tU';
    ...
    const imageryProviders = [
        ...
        // No Imagery (Transparent/White Background)
        new Cesium.ProviderViewModel({
            name: 'Kein Hintergrund',
            iconUrl: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/wcAAgAB/ed3vuwAAAAASUVORK5CYII=', // 1x1 white pixel
            tooltip: 'Disable all imagery layers',
            creationFunction: function() {
                return new Cesium.SingleTileImageryProvider({ url: '' }); // This effectively removes all imagery layers
            }
        })
    ];
    ...
    timeline: false,
      ...
      // Set Globe Base Color (Optional: Make it White)
...
const initialPosition = Cesium.Cartesian3.fromDegrees(12.495, 50.705, 600); // Long, Lat, Height
...
const LAT = 50.7174; // Latitude for Cologne, Germany
const LON = 12.488; // Longitude for Cologne, Germany

async function fetchCologneWeather() {
    const url = `https://api.openweathermap.org/data/2.5/weather?lat=${LAT}&lon=${LON}&appid=${API_KEY}&lang=de`;
    try {
    ...
    //viewer.scene.primitives.add(GoogletilesetKoeln);
      ...
      const wmsStarkregenExtremImageryProvider = new Cesium.WebMapServiceImageryProvider({
       ...
       //Netzplanung
          ...
          const Leerrohre = await Cesium.GeoJsonDataSource.load(resource1, {
            clampToGround: false,
          });
          
          viewer.dataSources.add(Leerrohre);
          // Höhenversatz.
          var entities = Leerrohre.entities.values;
          for (var i = 0; i < entities.length; i++) {
            var entity = entities[i];
          
            if (Cesium.defined(entity.polyline)) {
              // Get current positions and modify height
              var positions = entity.polyline.positions.getValue(Cesium.JulianDate.now());
              
              var adjustedPositions = positions.map(pos => {
              ...
              entity.polyline.positions = new Cesium.ConstantProperty(adjustedPositions);
          entity.name = `Leerrohre`
              // Set polyline color and width
              entity.polyline.material = new Cesium.ColorMaterialProperty(Cesium.Color.YELLOW.withAlpha(1));
              entity.polyline.width = 2.0; // Line thickness
            }
          }
        


          const resource2 = await Cesium.IonResource.fromAssetId(3113286);
          const Trassen = await Cesium.GeoJsonDataSource.load(resource2, {
            clampToGround: false,
          });
          
          viewer.dataSources.add(Trassen);
          // Höhenversatz.
          var entities2 = Trassen.entities.values;
          for (var i = 0; i < entities2.length; i++) {
            ...
            if (Cesium.defined(entity.polyline)) {
              ...
              entity.name = `Trassen`
              // Set polyline color and width
              entity.polyline.material = new Cesium.ColorMaterialProperty(Cesium.Color.BROWN.withAlpha(1));
              entity.polyline.width = 2.0; // Line thickness
            }
          }

          const resource3 = await Cesium.IonResource.fromAssetId(3113257);
const PointsDataSource = await Cesium.GeoJsonDataSource.load(resource3, {
  clampToGround: true, // Ensure points are clamped to terrain
});

viewer.dataSources.add(PointsDataSource);

// Set the name for each point
var entities = PointsDataSource.entities.values;
for (var i = 0; i < entities.length; i++) {
  var entity = entities[i];

  if (!Cesium.defined(entity.position)) continue; // Skip if no position

  entity.name = `Endverbraucher`; // Assign unique names

  // Optionally, log the names for debugging
  console.log(`Entity ${i + 1} named:`, entity.name);
}

const resource4 = await Cesium.IonResource.fromAssetId(3113762);
          const Verbindungen = await Cesium.GeoJsonDataSource.load(resource4, {
            clampToGround: false,
          });
          
          viewer.dataSources.add(Verbindungen);
          // Höhenversatz.
          var entities = Verbindungen.entities.values;
          for (var i = 0; i < entities.length; i++) {
            var entity = entities[i];
            ...
            if (Cesium.defined(entity.polyline)) {
              ...
              entity.polyline.positions = new Cesium.ConstantProperty(adjustedPositions);
          entity.name = `Verbindungen`
              // Set polyline color and width
              entity.polyline.material = new Cesium.ColorMaterialProperty(Cesium.Color.RED.withAlpha(1));
              entity.polyline.width = 2.0; // Line thickness
            }
          }
    ...
    PointsDataSource.show = false;
    Verbindungen.show = false; 
    Trassen.show = true;
    Leerrohre.show = false;
...
// Netzplanung
         //Trassen
         document.getElementById('TrassenCheckbox').addEventListener('change', () => {
            Trassen.show = !Trassen.show;
            });
          //Verbindungen
          document.getElementById('VerbindungenCheckbox').addEventListener('change', () => {
            Verbindungen.show = !Verbindungen.show;
            });
          //Leerrohre
          document.getElementById('LeerrohreCheckbox').addEventListener('change', () => {
            Leerrohre.show = !Leerrohre.show;
            });
 //Leerrohre
 document.getElementById('EndverbraucherCheckbox').addEventListener('change', () => {
    PointsDataSource.show = !PointsDataSource.show;
    });
